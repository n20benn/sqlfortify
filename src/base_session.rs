...
impl<T: io::Read + io::Write> BaseSession<T> {
    ...
    pub fn read_packet(&mut self, packet: &mut [u8]) -> io::Result<()> {
        if self.read_size > 0 && packet.len() != self.read_size { // TODO: add more comprehensive error checking by saving a hash of the packet?
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Unexpected internal error occurred while reading a packet".to_string())) // TODO: make error message clearer
        }
    ...
    pub fn write_packet(&mut self, packet: &[u8]) -> io::Result<()> {
        ...
        if self.write_size > 0 && packet.len() != self.write_size {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Unexpected internal error occurred while writing a packet".to_string())) // TODO: put better error string here
        }
