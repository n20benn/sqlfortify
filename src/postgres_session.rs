...
// This (and PostgresResponse) are currently just husks of implementations for SQLRequest and SQLResponse.
// They only need to extract out enough info to create a BasicRequestType, nothing more
...
impl<T: io::Read + io::Write> ClientSession<T> for PostgresClientSession<T> {
    ...
    fn send_request(&mut self, request: &Self::RequestType) -> io::Result<()> {
        send_request(&mut self.io, request, &mut self.state, &mut self.request_part_idx)
    }
    ...
    fn receive_response(&mut self) -> io::Result<Self::ResponseType> {
        ...
        match receive_response(&mut self.io, &mut response, &mut self.state, &mut self.request_failure) {
            Ok(()) => Ok(response),
...
impl<T: io::Read + io::Write> ServerSession<T> for PostgresServerSession<T> {
    ...
    fn send_response(&mut self, response: &Self::ResponseType) -> io::Result<()> {
        send_response(&mut self.io_device, response, &mut self.state, &mut self.response_part_idx)
    }
...
pub struct PostgresProxySession<C: io::Read + io::Write, S: io::Read + io::Write> {
    client_io: C,
    outstanding_request: Option<PostgresRequest>,
    outstanding_response: Option<PostgresRequest>, 
    recycled_requests: VecDeque<PostgresRequest>,
    recycled_responses: VecDeque<PostgresResponse>,
    request_failure: Option<bool>,
    request_part_idx: Option<usize>,
...
impl<C: io::Read + io::Write, S: io::Read + io::Write> ProxySession<C, S> for PostgresProxySession<C, S> {
    type RequestType = PostgresRequest;
    type ResponseType = PostgresResponse;
    ...
    fn server_send_response(&mut self, response: &Self::ResponseType) -> io::Result<()> {
        send_response(&mut self.server_io, response, &mut self.state, &mut self.response_part_idx)
    }
    ...
    fn client_receive_response(&mut self) -> io::Result<Self::ResponseType> {
        ...
        match receive_response(&mut self.client_io, &mut response, &mut self.state, &mut self.request_failure) {
            Ok(()) => Ok(response),
            Err(e) if e.kind() == io::ErrorKind::WouldBlock => {
                self.recycled_responses.push_front(response);
                Err(e)
    ...
    fn client_send_request(&mut self, request: &Self::RequestType) -> io::Result<()> {
        send_request(&mut self.client_io, request, &mut self.state, &mut self.request_part_idx)
    }
...
fn receive_request<T: io::Read>(io: &mut T, request: &mut PostgresRequest, state: &mut SessionState) -> io::Result<()> {
    ...
    match state {
        SessionState::Startup => {
            ...
            let pkt = read_packet(io, &mut request.raw_data, request.raw_len, request.packet_start, pkt_len)?;
            ...
            match parse_startup_req_packet(pkt) {
                ...
                Ok(RequestPacket::StartupMessage(version, user, params)) => {
                    ...
                    request.basic_info.database = Some(params.get("database").unwrap_or(&user).to_string());
                    if version != PostgresWireVersion::V3_0 {
                        request.basic_info.unsupported_version = true;
                    }
                    *state = SessionState::Normal;
                },
                Ok(_) => return Err(io::Error::new(io::ErrorKind::InvalidData, "unexpected packet received during connection startup")),
                Err(e) => return Err(io::Error::new(io::ErrorKind::InvalidData, e)),
            }
        },
        _ => {
            let header_bytes = read_packet(io, &mut request.raw_data, request.raw_len, request.packet_start, STANDARD_PKT_HDRLEN)?;
            ...
            match (*state, parse_standard_req_packet(pkt)) {
                ...
                (SessionState::Normal, Ok(RequestPacket::Parse(_,_,_) | RequestPacket::Bind(_,_,_,_,_) | RequestPacket::Execute(_,_) | RequestPacket::DescribePortal(_) 
                        | RequestPacket::DescribePrepared(_) | RequestPacket::ClosePortal(_) | RequestPacket::ClosePrepared(_) | RequestPacket::Flush)) => *state = SessionState::ExtendedQuery,
...
fn receive_response<T: io::Read>(io: &mut T, response: &mut PostgresResponse, state: &mut SessionState, request_failure: &mut Option<bool>) -> io::Result<()> {
    
    let header_bytes = read_packet(io, &mut response.raw_data, response.raw_len, response.packet_start, STANDARD_PKT_HDRLEN)?;
    let pkt_len = match read_standard_packet_len(header_bytes) {
        Ok((c, l)) => l,
        Err(e) => return Err(io::Error::new(io::ErrorKind::InvalidData, e)),
    };

    let pkt = read_packet(io, &mut response.raw_data, response.raw_len, response.packet_start, pkt_len)?;
    ...
    match (*state, parse_standard_resp_packet(pkt)) {
        ...
        (SessionState::Normal, Ok(ResponsePacket::CopyInResponse(_,_) | ResponsePacket::CopyBothResponse(_,_))) => *state = SessionState::CopyIn,
        (SessionState::Normal, Ok(_)) => (),
