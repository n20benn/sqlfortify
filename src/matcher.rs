use hashbrown::{HashMap,HashSet,hash_map::Entry}; // Switch to std HashMap once get_key_value_mut and try_insert are implemented
...
// NodeID used to abstract out implementation
...
impl<T:SqlToken> Node<T> {
    ...
    */
    ...
    fn get_child_update(&mut self, token:T, id_counter:&mut IDCounter) -> &mut Node<T> {
        ...
        // TODO: maybe write a blog post on this?
...
impl<T:SqlToken> SqlMatcher<T> {
    ...
    pub fn match_prefix_suffix(&self, sql_query:&Vec<T>) -> HashSet<NodeID> {
        ...
        // Also, if we don't have a prefix then DOOMED.
        let curr_param_node = match last_param_node { 
            Some(n) => n,
            None => return HashSet::new()
        };

        let fwd_param_ids = &curr_param_node.next_param_ids;
        let mut rev_param_ids:HashSet<NodeID> = HashSet::new();

        node = &self.rev_root;
        for token in sql_query.iter().rev() {
            rev_param_ids.extend(node.next_param_ids.iter());
